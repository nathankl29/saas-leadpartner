...
interface AppSettings { companyName: string; companyId: string; address: string; email: string; phone: string; bankDetails: string; invoiceFooter: string; legalNotice: string; primaryColor: string; monthlyGoal: number; dashboardLayout?: string[]; webhookUrl?: string; }
...
export default function App() {
  ...
  const [settings, setSettings] = useState<AppSettings>({
    ...
    webhookUrl: '', // Nouvelle option pour l'automatisation des emails
  });
  ...
  const handleSaveSettings = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newSettings = {
      ...settings,
      companyName: fd.get('companyName'), companyId: fd.get('companyId'), address: fd.get('address'), email: fd.get('email'), phone: fd.get('phone'), bankDetails: fd.get('bankDetails'), invoiceFooter: fd.get('invoiceFooter'), legalNotice: fd.get('legalNotice'), monthlyGoal: Number(fd.get('monthlyGoal')), webhookUrl: fd.get('webhookUrl'), primaryColor: BRAND_COLOR,
    };
  ...
  const applyEmailTemplate = (templateId: string, invoice: Partial<Invoice>, clientEmail: string) => {
      ...
      switch(templateId) {
          case 'std':
              subject = `Nouvelle Facture ${invId} - ${company}`;
              body = `Bonjour,\n\nVeuillez trouver ci-joint votre facture ${invId} d'un montant de ${amount} concernant nos prestations de génération de leads.\n\nNous restons à votre entière disposition pour toute question.\n\nCordialement,\nL'équipe ${company}`;
              break;
          case 'relance_1':
              subject = `Relance : Facture ${invId} en attente de règlement`;
              body = `Bonjour,\n\nSauf erreur ou omission de notre part, le règlement de la facture ${invId} d'un montant de ${amount} ne nous est pas encore parvenu.\n\nNous vous prions de bien vouloir procéder à son règlement dans les meilleurs délais.\nSi votre paiement a été effectué entre-temps, veuillez ne pas tenir compte de ce message.\n\nCordialement,\nL'équipe ${company}`;
              break;
          case 'relance_2':
              subject = `URGENT : Mise en demeure - Facture ${invId}`;
              body = `Bonjour,\n\nMalgré nos précédentes relances, nous constatons que la facture ${invId} d'un montant de ${amount} reste à ce jour impayée.\n\nNous vous mettons donc en demeure de procéder au règlement sous 48h. À défaut, des pénalités de retard pourront être appliquées conformément à nos conditions générales.\n\nDans l'attente de votre retour rapide,\n\nCordialement,\nLa Direction - ${company}`;
              break;
          default:
              break;
      }

      setEmailData({ to: clientEmail, subject, body, isSending: false, selectedTemplate: templateId });
  };
  ...
  const handleEmailInvoice = () => {
      ...
      applyEmailTemplate('std', currentInvoice, clientEmail);
  ...
  const handleSendEmailFromModal = async () => {
    ...
    try {
        ...
        // On charge html2pdf si ce n'est pas déjà fait
        if (!(window as any).html2pdf) {
        ...
        // On génère le PDF en Base64 (Format data URI)
        const pdfBase64 = await (window as any).html2pdf().set(opt).from(element).outputPdf('datauristring');

        // On envoie le payload au Webhook (Make.com ou Zapier)
        const payload = {
            ...
            pdf_attachment_base64: pdfBase64 // C'est ici que la magie opère
        };
  ...
  const handleExportData = () => {
      // Regrouper toutes les données actuelles du CRM
      const exportData = {
      ...
      // Convertir en format texte JSON lisible
      const dataStr = JSON.stringify(exportData, null, 2);
      
      // Créer un fichier "virtuel" (Blob)
      const dataBlob = new Blob([dataStr], { type: 'application/json' });
      const url = URL.createObjectURL(dataBlob);

      // Simuler un clic sur un lien de téléchargement
                   ...
                   {clientInvoices.map(inv => (
                     <div key={inv.id} onClick={() => { setCurrentInvoice(inv); setShowModal('invoice'); }} className="flex justify-between items-center p-3 bg-slate-50 border border-slate-100 rounded-xl cursor-pointer hover:border-[#01189B] hover:bg-white shadow-sm transition-all group">
                        ...
                        <div className="text-right">
                          ...
                          <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md ${inv.status === 'payee' ? 'bg-emerald-100 text-emerald-700' : inv.status === 'retard' ? 'bg-red-100 text-red-700' : 'bg-slate-200 text-slate-600'}`}>{inv.status}</span>
                        </div>
                     </div>
                   ))}
                 </div>
                ...
                <div className="rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${BRAND_COLOR} 0%, #0a2540 100%)` }}>
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="flex items-end gap-4 mb-4">
                            <h2 className="text-6xl md:text-7xl font-extrabold font-poppins">{renderCurrency(beneficeTotal)}</h2>
                        </div>
                        <div className="flex items-center gap-3">
                            <span className="bg-emerald-500/20 text-emerald-300 font-extrabold px-3 py-1 rounded-lg text-xl border border-emerald-500/30 font-mono">
                                {renderNumber(margeReellePourcent.toFixed(1))}% Marge Réelle
                            </span>
                            <span className="text-blue-200 text-sm font-medium">de marge totale sur {renderCurrency(totalBudget)} facturés.</span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10 relative z-10 border-t border-white/10 pt-8">
                        <div>
                            ...
                            <p className="text-2xl font-bold font-poppins text-white">{renderCurrency(fraisGestion)}</p>
                            <p className="text-xs text-blue-300 mt-1">{renderNumber(effectiveMargin)}% du CA total</p>
                        </div>
                        <div>
                            <p className="text-emerald-300/70 text-xs font-bold uppercase mb-1 tracking-wide">Bénéfice Arbitrage Média</p>
                            <p className="text-2xl font-bold font-poppins text-emerald-400">+{renderCurrency(beneficeArbitrage)}</p>
                        ...
                        <div className="bg-white/10 p-4 rounded-2xl border border-white/10 backdrop-blur-sm -mt-2">
                            ...
                            <p className="text-3xl font-extrabold font-poppins text-white">{renderNumber(leadsVendus)}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="space-y-6">
                        <div>
                            <div className="flex justify-between text-sm font-bold mb-2">
                                ...
                                <span className="font-poppins">{renderCurrency(beneficeTotal)}</span>
                            </div>
                        ...
                        <div>
                            <div className="flex justify-between text-sm font-bold mb-2">
                                ...
                                <span className="font-poppins text-orange-500">{renderCurrency(coutAchatEstime)}</span>
                            </div>
                            <div className="w-full bg-slate-100 rounded-full h-4 overflow-hidden">
        ...
        <div className="mt-12 pt-12 border-t border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {scenarios.map(sc => {
                    ...
                    const sc_cout = sc_leads * sc.currentRealCPL; 
                        ...
                        <div key={sc.id} className="bg-white rounded-2xl border border-slate-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] p-6 relative group hover:border-[#01189B] transition-colors">
                            ...
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 font-medium uppercase tracking-wide">CA: {renderCurrency(sc.totalBudget)}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                                    ...
                                    <span className={`font-extrabold text-lg font-poppins ${isGood ? 'text-emerald-600' : 'text-orange-600'}`}>{renderNumber(sc_margin.toFixed(1))}%</span>
                                </div>
                                <div className="flex justify-between items-end px-1">
                                    <div><p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">Bénéfice</p><p className="font-extrabold text-xl text-slate-800 font-poppins">{renderCurrency(sc_prof)}</p></div>
                                    <div className="text-right"><p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">Coût Média</p><p className="font-bold text-slate-500 font-mono">{renderCurrency(sc_cout)}</p></div>
                                </div>
                            </div>
                        </div>
                    );
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {simulations.map((sim) => {
                      ...
                      <tr key={sim.id} className="hover:bg-blue-50/30 transition-colors">
                        ...
                        <td className="px-6 py-5">
                          <span className={`font-extrabold px-3 py-1.5 rounded-lg font-mono ${sim.stats.margin >= 30 ? 'bg-emerald-50 text-emerald-700' : 'bg-orange-50 text-orange-700'}`}>
                            {renderCurrency(sim.stats.profit)} <span className="text-[10px] ml-1">({renderNumber(sim.stats.margin.toFixed(0))}%)</span>
                          </span>
                        </td>
                    ...
                    <div className="space-y-4">
                        {recentInvoices.map(inv => (
                            <div key={inv.id} className="flex justify-between items-center p-3 hover:bg-slate-50 rounded-xl transition-colors cursor-pointer" onClick={() => { setCurrentInvoice(inv); setShowModal('invoice'); }}>
                                <div className="flex items-center gap-3">
                                    <div className={`w-2 h-2 rounded-full ${inv.status === 'payee' ? 'bg-emerald-500' : inv.status === 'retard' ? 'bg-red-500' : 'bg-blue-400'}`}></div>
                                    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-min">
            {currentLayout.map(widgetId => {
                ...
                return (
                    ...
                    >
                        {/* Drag Handle Icon - Visually indicates draggability */}
      ...
      <form onSubmit={handleSaveSettings} className="grid grid-cols-1 gap-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-8">
              ...
              <div><label className="text-xs font-bold text-slate-400 uppercase tracking-wide">Pied de page / Conditions</label><textarea name="invoiceFooter" defaultValue={settings.invoiceFooter} className="w-full border-2 border-slate-100 bg-slate-50 p-3 rounded-xl mt-1 h-16 outline-none focus:border-[#01189B] focus:bg-white resize-none transition-colors text-slate-700" /></div>
              <div><label className="text-xs font-bold text-slate-400 uppercase tracking-wide flex justify-between">Ligne Légale (Bas de page) <span className="text-[10px] text-blue-500">Centré en bas</span></label><input name="legalNotice" defaultValue={settings.legalNotice || 'Entreprise individuelle non soumise à la TVA'} className="w-full border-2 border-slate-100 bg-slate-50 p-3 rounded-xl mt-1 outline-none focus:border-[#01189B] focus:bg-white font-medium transition-colors text-slate-700 text-sm" placeholder="Ex: Entreprise individuelle non soumise à la TVA" /></div>
              </div>
          </div>
        </div>

        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mt-8">
            <h3 className="font-extrabold text-lg mb-6 flex items-center gap-2 font-poppins border-b border-slate-100 pb-4 text-slate-800"><Link size={20} className="text-indigo-500" /> Automatisation Emails (Webhooks)</h3>
            <p className="text-sm text-slate-500 mb-6">Pour envoyer vos factures en PDF directement depuis `contact@leadpartner.ch`, connectez ce CRM à Make.com ou Zapier via un Webhook.</p>
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mt-8">
            <h3 className="font-extrabold text-lg mb-6 flex items-center gap-2 font-poppins border-b border-slate-100 pb-4 text-slate-800"><Download size={20} className="text-emerald-500" /> Sauvegarde & Export Data</h3>
            <p className="text-sm text-slate-500 mb-6">Téléchargez l'intégralité des données de votre CRM (Contacts, Factures, Scénarios, Notes...) au format JSON. Idéal pour garder une copie locale sécurisée avant une mise à jour.</p>
            
            <button type="button" onClick={handleExportData} className="px-6 py-3.5 bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 rounded-xl font-bold transition-colors flex items-center gap-2">
                <Download size={18}/> Télécharger Backup Complet (.json)
            </button>
        </div>

        <button type="submit" className="w-full text-white py-4 rounded-2xl font-bold hover:shadow-xl hover:-translate-y-1 flex items-center justify-center gap-2 transition-all text-lg mt-8" style={{ backgroundColor: BRAND_COLOR }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-10 no-print shrink-0 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.02)] z-10">
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="w-10 h-10 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-500 font-bold ml-2">
                 <Users size={18} />
             </div>
          </div>
        </header>
          ...
          <div className="bg-white p-8 rounded-3xl w-full max-w-4xl shadow-2xl border border-slate-100 animate-fade-in flex flex-col md:flex-row gap-8">
            ...
            <div className="w-full md:w-1/3 flex flex-col gap-6">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-2 font-medium">Le CRM va générer le PDF et l'envoyer via son propre serveur mail. Choisissez un modèle pour préremplir le message.</p>
                </div>
                
                <div className="space-y-3">
                    <h4 className="font-bold text-slate-700 text-sm uppercase tracking-wider mb-2">Modèles de Message</h4>
                    <button 
                        onClick={() => applyEmailTemplate('std', currentInvoice!, emailData.to)}
                        className={`w-full text-left p-3 rounded-xl border-2 transition-all ${emailData.selectedTemplate === 'std' ? 'border-[#01189B] bg-blue-50' : 'border-slate-100 hover:border-slate-200'}`}
                    >
                        <p className={`font-bold text-sm ${emailData.selectedTemplate === 'std' ? 'text-[#01189B]' : 'text-slate-700'}`}>Standard (Nouvelle Facture)</p>
                        <p className="text-xs text-slate-500 mt-1">Message poli et classique d'envoi.</p>
                    </button>
                    <button 
                        onClick={() => applyEmailTemplate('relance_1', currentInvoice!, emailData.to)}
                        className={`w-full text-left p-3 rounded-xl border-2 transition-all ${emailData.selectedTemplate === 'relance_1' ? 'border-orange-500 bg-orange-50' : 'border-slate-100 hover:border-slate-200'}`}
                    >
                        <p className={`font-bold text-sm ${emailData.selectedTemplate === 'relance_1' ? 'text-orange-700' : 'text-slate-700'}`}>Relance Aimable (+10 Jours)</p>
                        <p className="text-xs text-slate-500 mt-1">Sauf erreur ou omission de notre part...</p>
                    </button>
                    <button 
                        onClick={() => applyEmailTemplate('relance_2', currentInvoice!, emailData.to)}
                        className={`w-full text-left p-3 rounded-xl border-2 transition-all ${emailData.selectedTemplate === 'relance_2' ? 'border-red-500 bg-red-50' : 'border-slate-100 hover:border-slate-200'}`}
                    >
                        <p className={`font-bold text-sm flex items-center gap-1.5 ${emailData.selectedTemplate === 'relance_2' ? 'text-red-700' : 'text-slate-700'}`}><AlertTriangle size={14}/> Mise en demeure</p>
                        <p className="text-xs text-slate-500 mt-1">Message ferme de rappel d'impayé.</p>
                    </button>
                </div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mt-auto">
                    ...
                    <p className="text-[10px] text-slate-400">La facture est envoyée via l'API EmailJS. L'expéditeur affiché sera <strong>{settings.email}</strong>.</p>
                </div>
            </div>
            ...
            <div className="flex-1 space-y-5 bg-slate-50 p-6 rounded-2xl border border-slate-100 shadow-inner">
              ...
              <div className="pt-4 flex gap-4 border-t border-slate-200 mt-6">
                ...
                <button disabled={emailData.isSending} onClick={handleSendEmailFromModal} className="flex-1 py-3.5 text-white rounded-xl font-bold hover:shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-70" style={{ backgroundColor: BRAND_COLOR }}>
                  {emailData.isSending ? <><Loader size={18} className="animate-spin"/> Transmission API...</> : <><Send size={18}/> {emailData.selectedTemplate.includes('relance') ? 'Envoyer la Relance' : 'Envoyer la Facture'}</>}
                </button>
              </div>
            </div>
          </div>
